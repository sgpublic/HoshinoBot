...
sv = Service("pcr-rank", help_=sv_help, bundle="pcr查询")

server_addr = "https://raw.githubusercontent.com/pcrbot/pcr-rank_data/master/rank/"
resize_pic = False
config = PCR_RANK
...
async def update_rank_pic_cache(force_update: bool):
    ...
    for conf_name in config_names:
        ...
        for img_name in rank_config["files"]:
            if not force_update:
                if path.exists(
                        path.join(path.abspath(path.dirname(__file__)), "cache", "pic", f"{conf_name}_{img_name}")):
                    continue
            ...
            with open(path.join(path.abspath(path.dirname(__file__)), "cache", "pic", f"{conf_name}_{img_name}"),
                      "ab") as fp:
                fp.seek(0)
                fp.truncate()
